...
flextable(sign_test_pairwise_table) %>%
  ...
  set_header_labels(values=list(boscc_subscale="BOSCC Subscale", boscc_type="BOSCC Type",
                                ...
                                group_diff_test="Overall Test P-value",
                                pairwise_diff_test="Pairwise Test P-value",
                                pairwise_effect_size="Pairwise Cohen's D")) %>%
  merge_v(j=1:3) %>%
  valign(valign = "top") %>%
  hline() %>%
  ...
  autofit() %>%
  add_footer_row(values = "Overall Test provides uncorrected p-values for Mann Whitney test for any median differences between clusters\nPairwise Test provides FDR-corrected p-values for pairwise tests for any median differences.\nSignificant pairwise differences highlighted in blue.\nPairwise Cohen's D provides Cohen's D effect sizes for mean differences between pairs. \nOnly results with significant overall tests provided for brevity.", colwidths = 7) %>%
  fix_border_issues() %>%
